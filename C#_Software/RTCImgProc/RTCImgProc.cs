...
namespace rt_caiman
{
    public partial class RTCImgProc : Form
    {
        ...
        public static byte[] dispImg_f = new byte[dispImgSize];
        public static byte[] dispImg_buf = new byte[dispImgSize];
        public static byte[] dispImg_ero = new byte[dispImgSize];
        public static byte[] dispImg_open = new byte[dispImgSize];
        public static byte[] dispRGBImg = new byte[dispImgSize * 3];

        public static int dispTraceW = 400;
        public static int dispTraceH = 309;
        ...
        public static int dispDecodeW = 400;
        public static int dispDecodeH = 192;
        public static int dispDecodeSize = dispDecodeW * dispDecodeH;
        public static byte[] dispRGBDecode = new byte[dispDecodeSize * 3];
        ...
        public static int[] Tid = new int[103];
        public static int[] Max = new int[103];
        public static int[] Min = new int[103];

        public static int cur_group = 11;

        public static byte[] color = new byte[7 * 3];

        public static int[] true_labels = new int[1000];
        ...
        private void SetTraceImage(Bitmap image)
        {
            if (this.picDisplay.InvokeRequired)
            {
                SetImageCallback callback = new SetImageCallback(SetTraceImage);
        ...
        private void SetDecode(Bitmap image)
        {
            if (this.picDecode.InvokeRequired)
            {
                SetImageCallback callback = new SetImageCallback(SetDecode);
                this.BeginInvoke(callback, new object[] { image });
            }
            else
            {
                picDecode.Image = image;
            }
        }
        ...
        public void displayRGBDecode(byte[] dispImg)
        {
            // Create a new Bitmap
            Bitmap bmp = new Bitmap(dispDecodeW, dispDecodeH, System.Drawing.Imaging.PixelFormat.Format24bppRgb);
                ...
                if (recvId_val > frameId_val)
                {
                    ...
                    for (j = 0; j < dispImgH; ++j)
                    {
                        for (i = 0; i < dispImgW; ++i)
                        {
                            ...
                            // Rescaling
                            //dispImg[j * dispImgW + i] = (byte) (dispImg[j * dispImgW + i] * 2.4);
                        }
                    }
                    ...
                    for (j = 0; j < dispImgH; ++j)
                    {
                        for (i = 0; i < dispImgW; ++i)
                        {
                            ...
                            for (fj = -1; fj <= 1; ++fj)
                            {
                                for (fi = -1; fi <= 1; ++fi)
                                {
                                    if (((j+fj)>=0)&&((j+fj)<dispImgH)&&((i+fi)>=0)&&((i+fi)<dispImgW))
                                    {
                                        val += dispImg[j * dispImgW + i];
                                    }
                                }
                            }
                            dispImg_f[j * dispImgW + i] = (byte) (val / 9);
                        }
                    }
                    */
                    ...
                    for (j = 0; j < dispImgH; ++j)
                    {
                        for (i = 0; i < dispImgW; ++i)
                        {
                            ...
                            for (fi = -1; fi <= 1; ++fi)
                            {
                                if (((i+fi)>=0)&&((i+fi)<dispImgW))
                                {
                                    if (dispImg_f[j * dispImgW + i + fi] < val)
                                    {
                                        val = dispImg_f[j * dispImgW + i + fi];
                                    }
                                }
                            }
                            dispImg_buf[j * dispImgW + i] = (byte) val;
                        }
                    }

                    for (j = 0; j < dispImgW; ++j)
                    {
                        for (i = 0; i < dispImgH; ++i)
                        {
                            val = 255;
                            for (fj = -1; fj <= 1; ++fj)
                            {
                                if (((j+fj)>=0)&&((j+fj)<dispImgW))
                                {
                                    if (dispImg_buf[(j + fj) * dispImgW + i] < val)
                                    {
                                        val = dispImg_buf[(j + fj) * dispImgW + i];
                                    }
                                }
                            }
                            dispImg_ero[j * dispImgW + i] = (byte) val;
                        }
                    }
                        ...
                        for (j = 0; j < dispImgH; ++j)
                        {
                            for (i = 0; i < dispImgW; ++i)
                            {
                                ...
                                for (fi = -1; fi <= 1; ++fi)
                                {
                                    if (((i+fi)>=0)&&((i+fi)<dispImgW))
                                    {
                                        if (dispImg_ero[j * dispImgW + i + fi] < val)
                                        {
                                            val = dispImg_ero[j * dispImgW + i + fi];
                                        }
                                    }
                                }
                                dispImg_buf[j * dispImgW + i] = (byte) val;
                    ...
                    */
                        ...
                        for (i = 0; i < dispImgW; ++i)
                        {
                            ...
                            for (fj = -9; fj <= 9; ++fj)
                            {
                                for (fi = -9; fi <= 9; ++fi)
                                {
                                    if (((j+fj)>=0)&&((j+fj)<dispImgH)&&((i+fi)>=0)&&((i+fi)<dispImgW))
                                    {
                                        val = dispImg_ero[j * dispImgW + i];
                                    }
                                }
                            }
                            dispImg_open[j * dispImgW + i] = (byte) val;
                        }
                    }
                    ...
                    for (j = 0; j < dispImgH; ++j)
                    {
                        for (i = 0; i < dispImgW; ++i)
                        {
                            dispImg[j * dispImgW + i] = (byte) (dispImg[j * dispImgW + i] - dispImg_open[j * dispImgW + i]);
                        }
                    }
                    */
        ...
        private void btnStartMCorre_Click(object sender, EventArgs e)
        {
            ...
            byte[] parameter = new byte[86272 + 94300];
            byte[] fileBytes;
            ...
            if (recordingIsOn == 0)
            {
                ...
                for (i = 0; i < 94300; ++i)
                {
                    parameter[i + 86272] = fileBytes[i];
                }

                try
                {
                    RTCImgProc.Send(TCPsocket, parameter, 0, 86272 + 94300, 10000);
                }
                catch (Exception ex)
                {
                    Console.WriteLine("Unexpected exception: {0}", ex.ToString());
                ...
                // Load ground truth labels of the decoding.
                inputFilename = work_path.Substring(0, loc) + "file\\label\\labels.txt";
                fileLines = File.ReadAllLines(inputFilename);
                for (i = 0; i < 1000; ++i)
                {
                    true_labels[i] = Convert.ToInt32(fileLines[i]);
                }
                
                // Load selected cells from sel_tiles.txt file.
                inputFilename = work_path.Substring(0, loc) + "file\\contour\\sel_tiles.txt";
                fileLines = File.ReadAllLines(inputFilename);
                for (i = 0; i < 103; i++)
                {
                    Tid[i] = Convert.ToInt32(fileLines[i]);
                    Min[i] = MaxMin[Tid[i] - 1, 0];
                    Max[i] = MaxMin[Tid[i] - 1, 1];
                }
        ...
        private void ExecuteMCorreProcess()
        {
            ...
            int c;
            int row, col;
            int [] pre_trace = new int[103];
            int [] cur_trace = new int[103];
            byte[] cmd = new byte[4];
            byte[] send = new byte[4];
            byte[] recv = new byte[1440];
            // ImgSize: 512x512 = 262144
            ...
            int[] norm_trace = new int[103 * 1000];

            int[] predict_output = new int[1000];

            // Parameters for Group-Max
            /*
            // Hipp15
            int[] tid = new int[63] {
                5,   8,  28,  34,  40,  44,  49,  64,  68,
            ...
            int[] max = new int[63] {
                993, 1507, 1033,  946, 1074, 1533, 1224, 1270, 2005,
            ...
            */
            ...
            // Hipp18
            int[] tid = new int[63] {
                861, 901, 471, 671, 78, 72, 282, 56, 200,
                33, 238, 150, 463, 419, 951, 961, 711, 183,
                297, 704, 273, 236, 535, 38, 7, 2, 193,
                284, 1, 929, 22, 479, 127, 963, 54, 974,
                181, 310, 210, 907, 341, 3, 192, 47, 779,
                46, 293, 188, 567, 30, 170, 678, 515, 105,
                6, 559, 953, 906, 13, 217, 453, 657, 155 };

            int[] min = new int[63] {
                79, 115, 87, 114, 0, 132, 53, 1, 33,
                0, 323, 9, 15, 23, 114, 10, 10, 367,
                155, 46, 3, 0, 23, 20, 58, 9, 188,
                0, 0, 14, 25, 4, 3, 61, 0, 7,
                147, 21, 4, 20, 21, 25, 269, 13, 12,
                1, 1, 414, 14, 15, 32, 3, 6, 84,
                14, 0, 7, 2, 6, 208, 13, 0, 6 };

            int[] max = new int[63] {
                2388, 2335, 2222, 2132, 1944, 1869, 1820, 1788, 1765,
                1759, 1757, 1725, 1719, 1690, 1689, 1623, 1600, 1561,
                1535, 1522, 1519, 1502, 1493, 1492, 1492, 1468, 1468,
                1445, 1440, 1408, 1402, 1385, 1383, 1364, 1327, 1322,
                1319, 1318, 1315, 1289, 1268, 1260, 1248, 1242, 1237,
                1229, 1224, 1216, 1179, 1160, 1156, 1155, 1150, 1144,
                1139, 1130, 1130, 1127, 1124, 1122, 1120, 1106, 1093 };
            */
            ...
            do
            {
                ...
                if (recvId_val > frameId_val)
                {
                    ...
                    predict_output[ExecuteTime - 1] = recv_buf[ImgSize + 8 + 2048] + (recv_buf[ImgSize + 8 + 2048 + 1] << 8);
                    //Console.WriteLine(predict_output[ExecuteTime - 1].ToString());
                    ...
                    {
                        // Set up display with received images.
                        /*
                        for (j = 0; j < dispImgH; ++j)
                        {
                            for (i = 0; i < dispImgW; ++i)
                            {
                                ...
                                for (c = 0; c < 3; ++c)
                                {
                                    if ((dispRGBImg[(j * dispImgW + i) * 3 + c] * 15) > 255)
                                    {
                                        dispRGBImg[(j * dispImgW + i) * 3 + c] = 255;
                                    }
                                    else
                                    {
                                        dispRGBImg[(j * dispImgW + i) * 3 + c] = (byte)(dispRGBImg[(j * dispImgW + i) * 3 + c] * 15);
                                    }
                                }
                            }
                        }
                        */
                        ...
                        // Set up display with received traces.
                        cell = 0;
                        for (j = 0; j < 32; ++j)
                        {
                            if ((j % 2) == 0)
                                ...
                                for (i = 0; i < 32; ++i)
                                {
                                    ...
                                    for (sj = 0; sj < 16; ++sj)
                                    {
                                        for (si = 0; si < 16; ++si)
                                        {
                                            ...
                                            for (c = 0; c < 3; ++c)
                                            {
                                                dispRGBImg[(row * dispImgW + col) * 3 + c] = pixel;
                                            }
                                        }
                                    }
                                    ...
                                    if (contourMaps[Tid[cell] - 1, j, i] == 1)
                                    {
                                        ...
                                        if ((i == 0) || (i == 24) || (j == 0) || (j == 24))
                                        {
                                            for (c = 0; c < 3; ++c)
                                            {
                                                ...
                                                dispRGBImg[(row * dispImgW + col) * 3 + c] = color[2 * 3 + c];
                                            }
                                        }
                                        else
                                        {
                                            if ((contourMaps[Tid[cell] - 1, j - 1, i] + contourMaps[Tid[cell] - 1, j + 1, i] +
                                                contourMaps[Tid[cell] - 1, j, i - 1] + contourMaps[Tid[cell] - 1, j, i + 1]) < 4)
                                            {
                                                for (c = 0; c < 3; ++c)
                                                {
                                                    //dispRGBImg[(row * dispImgW + col) * 3 + c] = color[(cell % 7) * 3 + c];
                    ...
                    for (cell = 0; cell < 103; ++cell)
                    {
                        ...
                        norm_trace[cell * 1000 + (ExecuteTime - 1)] = (trace[Tid[cell] - 1] - Min[cell]) * 255 / (Max[cell] - Min[cell]);
                    }
                        ...
                        for (cell = 0; cell < 103; ++cell)
                        {
                            for (i = 0; i < ExecuteTime; ++i)
                            {
                                for (j = 0; j < 3; ++j)
                                {
                                    for (c = 0; c < 3; ++c)
                                    {
                                        dispRGBTrace[((cell * 3 + j) * dispTraceW + i) * 3 + c] = (byte) norm_trace[cell * 1000 + i];
                                    }
                                }
                            }
                        }
                        ...
                        for (cell = 0; cell < 103; ++cell)
                        {
                            for (i = 0; i < 400; ++i)
                            {
                                for (j = 0; j < 3; ++j)
                                {
                                    for (c = 0; c < 3; ++c)
                                    {
                                        dispRGBTrace[((cell * 3 + j) * dispTraceW + i) * 3 + c] = (byte) norm_trace[cell * 1000 + i + ExecuteTime - 400];
                                    }
                                }
                            }
                        }
                    ...
                    // Display one frame of decoding plo
                    for (j = 0; j < dispDecodeH; ++j)
                    {
                        for (i = 0; i < dispDecodeW; ++i)
                        {
                            for (c = 0; c < 3; ++c)
                            {
                                dispRGBDecode[(j * dispDecodeW + i) * 3 + c] = 255;
                            }
                        }
                    }
                    ...
                    if (ExecuteTime <= 400)
                    {
                        for (i = 0; i < ExecuteTime; ++i)
                        {
                            for (j = 0; j < 7; ++j)
                            {
                                for (c = 0; c < 3; ++c)
                                {
                                    //dispRGBDecode[((7 + (24 - true_labels[i]) * 6 + j) * dispDecodeW + i) * 3 + c] = color[0 * 3 + c];
                                    //dispRGBDecode[((6 + (24 - predict_output[i]) * 6 + j) * dispDecodeW + i) * 3 + c] = color[1 * 3 + c];

                                    dispRGBDecode[((12 + (true_labels[i] - 1) * 7 + j) * dispDecodeW + i) * 3 + c] = color[0 * 3 + c];
                                    dispRGBDecode[((11 + (predict_output[i] - 1) * 7 + j) * dispDecodeW + i) * 3 + c] = color[1 * 3 + c];
                                }
                            }
                        }
                    }
                    else
                    {
                        for (i = 0; i < 400; ++i)
                        {
                            for (j = 0; j < 7; ++j)
                            {
                                for (c = 0; c < 3; ++c)
                                {
                                    //dispRGBDecode[((7 + (24 - true_labels[ExecuteTime - 400 + i]) * 6 + j) * dispDecodeW + i) * 3 + c] = color[0 * 3 + c];
                                    //dispRGBDecode[((6 + (24 - predict_output[ExecuteTime - 400 + i]) * 6 + j) * dispDecodeW + i) * 3 + c] = color[1 * 3 + c];

                                    dispRGBDecode[((12 + (true_labels[ExecuteTime - 400 + i] - 1) * 7 + j) * dispDecodeW + i) * 3 + c] = color[0 * 3 + c];
                                    dispRGBDecode[((11 + (predict_output[ExecuteTime - 400 + i] - 1) * 7 + j) * dispDecodeW + i) * 3 + c] = color[1 * 3 + c];
                                }
                            }
                        }
                    }

                    displayRGBDecode(dispRGBDecode);
